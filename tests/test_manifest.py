...
from onnxvoice.types import Artifact, AudioResult, CatalogItem
...
def test_manifest_round_trip_preserves_runtime_metadata(tmp_path):
    ...
    assert json.loads((installation.path / "manifest.json").read_text())[
        "schema"
    ] == 2
...
def test_audio_result_is_canonical_float32_mono():
    result = AudioResult(np.array([1, 2], dtype=np.float64), 24000)
    assert result.audio.dtype == np.float32
    assert result.audio.ndim == 1

    with pytest.raises(ValueError, match="one-dimensional"):
        AudioResult(np.array([[1, 2]], dtype=np.float64), 24000)
